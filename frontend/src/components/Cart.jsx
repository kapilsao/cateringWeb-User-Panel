...
//     navigate('/cnf', { state : deliveryMode});
...
import React, { useEffect, useState } from "react";
import { FaPlus, FaMinus, FaStar, FaUtensils, FaHeart } from "react-icons/fa";
import useCartStore from "../store/cartStore";

export default function Cart() {
  const [deliveryMode, setDeliveryMode] = useState("onsite");
  const [quantity, setQuantity] = useState(1); // Quantity entered by the user
  const { cart, setCart, updateItemQuantity, removeItem } = useCartStore();
  const total = cart.reduce((acc, item) => acc + item.price * item.quantity, 0); // Cart total
  const navigate = useNavigate();
  ...
  const handleOnClick = () => {
    ...
    localStorage.setItem("Ototal", total),
    localStorage.setItem("mode", deliveryMode)
    navigate("/cnf",);

  };

  const totalOrderValue = total * quantity; // Total order value calculation
  ...
  return (
    <div className="max-w-md mx-auto mt-28 mb-28 bg-white min-h-screen">
      ...
      {/* Quantity Input & Total Order Value */}
      <div className="flex items-center justify-between px-4 mt-6 mb-4">
        ...
        <span className="text-[#5c2617] font-semibold">
