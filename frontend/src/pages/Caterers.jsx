...
//     localStorage.setItem("caterer name",name)
...
//               onClick={() => handleCardClick(cateror._id, cateror.name)}>
//               <img
...
export default function Caterors() {
  const [visibleCaterers, setVisibleCaterers] = useState(4); // Initial number of visible caterers
  const [caterers, setCaterers] = useState([]);
  ...
  // Filtered caterers based on the search query
  const filteredCaterers = caterers.filter((cateror) =>
    cateror.name.toLowerCase().includes(searchQuery.toLowerCase())
  );
    ...
    <div className="bg-gray-100 mt-16 mb-14 min-h-screen p-8">
      ...
      <section className="mt-8">
        ...
        <div className="flex items-center bg-white rounded-lg shadow-md p-3 mt-4 max-w-md mx-auto">
          ...
          <input
            type="text"
            placeholder="Filter Your Caterer"
        ...
        {/* Caterer Cards */}
        <div className="grid grid-cols-2 gap-6 mt-6 sm:grid-cols-3 lg:grid-cols-4">
          {filteredCaterers.slice(0, visibleCaterers).map((cateror) => (
            <div
              key={cateror._id}
              className="bg-white rounded-lg shadow-lg overflow-hidden text-center transform transition-transform duration-300 hover:scale-105"
              onClick={() => handleCardClick(cateror._id, cateror.name)}
        ...
        {/* See More Button */}
        {visibleCaterers < filteredCaterers.length && (
          <div className="text-right mt-4 mb-10">
            <button
              className="text-orange-500 font-medium hover:text-orange-600 flex items-center"
              onClick={handleSeeMore}
